...
def hub_tests():
    ...
    # result in nothing happening
    reqpkt = mk_pkt("30:00:00:00:00:01", "10:00:00:00:00:03", '172.16.42.2','172.16.42.2')
    s.expect(PacketInputEvent("eth1", reqpkt, display=Ethernet), "An Ethernet frame should arrive on eth2 with destination address the same as eth1's MAC address")
    s.expect(PacketInputTimeoutEvent(1.0), "The hub should not do anything in response to a frame arriving with a destination address referring to the hub itself.")
    return s
    ...
    s.expect(PacketOutputEvent("eth0", testpkt, "eth1", testpkt, display=Ethernet), "The Ethernet frame with a broadcast destination address should be forwarded out ports eth0 and eth1")
